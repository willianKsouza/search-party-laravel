...
@section('content')
    <x-container>
        <div class="max-w-md mx-auto mt-10 bg-gray-800 shadow-lg rounded-lg p-8 space-y-6">
            <form x-data="users" x-init="setUser({{ json_encode($user) }})" x-on:submit.prevent="update({{ $user->id }})"
                class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input x-model="form.data.user_name" type="text" id="user_name" name="user_name"
                    ...
                    <div>
                        <p x-show="form.errors.user_name" class="text-xs text-red-500 mt-1" x-text="form.errors.user_name">
                        </p>
                    </div>
                </div>
                <div>
                    <label for="email" class="block text-sm font-medium text-orange-500 mb-1">Email</label>
                    <input x-model="form.data.email" type="email" id="email" name="email"
                ...
                <div>
                    <p x-text="form.warning" class="text-sm text-green-500 mt-1"></p>
                </div>
                <div>
            ...
            <form class="space-y-6">
                <div>
                    ...
                    <label for="password" class="block text-sm font-medium text-orange-500 mb-1">Digite sua senha
                        atual</label>
                    <input x-model="form.data.password" type="password" id="password" name="password"
                        placeholder="********"
                        class="w-full text-white border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                    <div>
                        <p x-show="form.errors.password" class="text-xs text-red-500 mt-1" x-text="form.errors.password">
                        </p>
                    </div>
                </div>
                <div>
                    <label for="password_confirmation" class="block text-sm font-medium text-orange-500 mb-1">Digite sua
                        nova senha</label>
                    <input x-model="form.data.password_confirmation" type="password" id="password_confirmation"
                ...
                <div>
                    <label for="password_confirmation" class="block text-sm font-medium text-orange-500 mb-1">Confirme sua
                        nova senha</label>
                    <input x-model="form.data.password_confirmation" type="password" id="password_confirmation"
                        name="password_confirmation" placeholder="********"
                        class="w-full text-white border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
